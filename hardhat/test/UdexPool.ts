...
describe("UdexPool", function () {
    ...
    describe("swap", function () {
        ...
        it("swap and withdraw too much token1", async function () {
            ...
            await expect(pool.connect(account1).swap(0, amountOut, account2.address)).to.be.revertedWith('UdexPool: K');
        });
        ...
        it("swap without input", async function () {
            ...
            const reserveIn = await pool.reserve0();
            const reserveOut = await pool.reserve1();
            await expect(pool.connect(account1).swap(0, 1, account2.address)).to.be.revertedWith('UdexPool: INSUFFICIENT_INPUT_AMOUNT');
        });         
    })
})